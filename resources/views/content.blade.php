...
<div class="card-header">メモ内容
    <a href="/edit/{{ $memo[0]['id'] }}" class="text-secondary ml-1"><i class="far fa-edit"></i></a>
</div>
    ...
    <div class="mb-2 d-flex justify-content-center">
        ...
        <div class="text-right ml-3">
            ...
            <div class="mb-2">
                <a href="{{ $memo[0]['url'] }}" class="text-danger btn-lg"><i class="fab fa-youtube"></i></a>
            </div>
        </div>
    </div>

    <div class="m-3 scroll h-220px border-top border-bottom">
        {!!nl2br(e($memo[0]['content']))!!}
    </div>

    <div class="scroll h-60px">登録タグ:
        @foreach($tags as $tag)
            @if(in_array($tag['id'], $include_tags))
                <a class="ml-2" href="/?tag={{ $tag['id'] }}">
                    {{ $tag['name'] }} ({{ $tag['count'] }})
                </a>
            @endif
        @endforeach
